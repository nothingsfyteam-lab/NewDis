(function () {
  ...
  let lkRoom = null;
  let peerConnections = {}; // userId -> PC (will be deprecated by LiveKit)
  ...
  function updateActiveNavState() {
    ...
    document.querySelectorAll('.server-icon').forEach(el => {
      const id = el.getAttribute('onclick')?.match(/'([^']+)'/)?.[1];
      const isActive = currentServerId === id || (!currentServerId && el.getAttribute('onclick')?.includes('openHome'));
      el.classList.toggle('active', isActive);
    });
  }
  ...
  function connectSocket() {
    ...
    socket.on('incoming-call', async (data) => {
      ...
      if (modal) {
        ...
        document.getElementById('accept-call-btn').onclick = async () => {
          ...
          socket.emit('answer-call', { to: data.from });

          document.getElementById('call-target-name').textContent = data.callerName;
          document.getElementById('call-target-avatar').textContent = data.callerName[0].toUpperCase();

          await connectToLiveKitCall(data.callRoomId, data.withVideo);
        };
        ...
        document.getElementById('reject-call-btn').onclick = () => {
          ...
          stopRingtone();
          socket.emit('reject-call', { to: data.from });
          showNotification('Call Declined', 'You declined the call', 'info');
        };
      }
    });
    ...
    socket.on('call-accepted', (data) => {
      ...
      showNotification('Call Answered', 'Connecting...', 'info');
    });

    socket.on('call-rejected', (data) => {
      stopRingtone();
      showNotification('Call Rejected', 'The user declined your call.', 'error');
      endCall();
    });

    socket.on('call-ended', (data) => {
      showNotification('Call Ended', 'The other user ended the call.', 'info');
      endCall();
    });
    ...
    async function renderSidebarNav() {
      try {
        ...
        const rail = document.getElementById('server-list-rail');
        if (!rail) return;

        rail.innerHTML = servers.map(s => `
        <div class="server-icon ${currentServerId === s.id ? 'active' : ''}"
             onclick="window.app.openServer('${s.id}')"
             title="${esc(s.name)}">
          ${s.icon ? `<img src="${s.icon}" style="width:100%;height:100%;border-radius:inherit;object-fit:cover;">` : s.name.substring(0, 2).toUpperCase()}
        </div>
      `).join('');
    ...
    window.app.renderSidebarNav = renderSidebarNav;

    window.app.openServer = async (serverId) => {
      currentServerId = serverId;
      currentContext = { type: 'server', id: serverId, data: null };
      ...
      if (server) {
        currentContext.data = server;
        document.getElementById('nav-header-title').textContent = server.name;
        const codeEl = document.getElementById('nav-invite-code');
        if (codeEl) {
          codeEl.textContent = server.invite_code || '---';
    ...
    window.app.showCreateServerModal = () => {
      const name = prompt('Server Name:');
      if (name) {
        api('/api/servers', { method: 'POST', body: { name } }).then(s => {
          renderSidebarNav();
          window.app.openServer(s.id);
        });
      }
    };
    ...
    try {
      ...
      let html = `
        <div class="sidebar-section">
          <div class="sidebar-section-header" style="padding: 0 8px;">
            TEXT CHANNELS
            <button class="section-btn" onclick="window.app.createChannelPrompt('text')">+</button>
          </div>`;
      ...
      textChannels.forEach(ch => {
        html += `
          <div class="nav-item ${currentChannelId === ch.id ? 'active' : ''}"
      ...
      html += `
          <div class="sidebar-section-header" style="padding: 0 8px; margin-top: 16px;">
            VOICE CHANNELS
            <button class="section-btn" onclick="window.app.createChannelPrompt('voice')">+</button>
          </div>`;

      voiceChannels.forEach(ch => {
        const usersInRoom = voiceStates[ch.id] || [];
        html += `
            <div class="nav-item voice-channel ${currentVoiceChannel === ch.id ? 'active' : ''}" 
  ...
  window.app.createChannelPrompt = async (type = 'text') => {
    ...
    if (name) {
      await api(`/api/channels`, { method: 'POST', body: { name, type, server_id: currentServerId } });
      loadChannels(currentServerId);
    }
  };
  ...
  function setTheme(t) {
    ...
    // Update active state in UI if buttons exist
  ...
  async function showDeleteConfirmationModal(type, id, name) {
    ...
    modal.style.display = 'flex';
    document.getElementById('modal-title').textContent = `Delete Server`;
  ...
  async function joinVoice(targetId = null) {
    ...
    try {
      const success = await connectToLiveKit(id);
      if (!success) return;
  ...
  // --- LiveKit Logic ---
  async function connectToLiveKit(channelId) {
    if (lkRoom) {
    ...
    try {
      ...
      lkRoom.on(Livekit.RoomEvent.TrackSubscribed, (track, publication, participant) => {
        ...
        if (track.kind === 'audio') {
          ...
          const playAudio = () => {
            el.play().catch(err => {
              console.warn("[LiveKit] Audio play blocked, waiting for interaction", err);
            });
          };
          playAudio();
          // Fallback if blocked
      ...
      console.log('[LiveKit] Connected to', lkRoom.name);

      // Default to voice only for now to match current app behavior
      ...
      if (e.message.includes('configured')) {
        showNotification('Setup Required', 'LiveKit is not configured on the server yet. Please add LIVEKIT_URL, LIVEKIT_API_KEY, and LIVEKIT_API_SECRET to environment variables.', 'error');
      } else {
        showNotification('Connection Error', 'Failed to connect to LiveKit: ' + e.message, 'error');
      }
      return false;
    }
  }

  // (Removed old Mesh WebRTC handlers: initiatesConnection, handleOffer, handleAnswer, handleCandidate)
  ...
  window.playRingtone = () => {
    ...
    if (r) {
      r.currentTime = 0;
      r.loop = true;
      r.play().catch(e => console.log("Audio play blocked"));
    }
  };
  ...
  function showNotification(title, message, type) {
    // Expose to window immediately for any inline handlers
  ...
  async function startCall(targetId, withVideo = true) {
    ...
    const callRoomId = `call-${currentUser.id}-${targetId}-${Date.now()}`;
    ...
    socket.emit('call-user', { to: targetId, callRoomId, withVideo });

    await connectToLiveKitCall(callRoomId, withVideo);
  }

  async function connectToLiveKitCall(roomId, withVideo) {
    if (lkRoom) {
      await lkRoom.disconnect();
      lkRoom = null;
    }
    ...
    try {
      ...
      await lkRoom.connect(data.url, data.token);
      console.log('[LiveKit Call] Connected to room', roomId);
      ...
      showNotification('Call Error', 'Failed to connect to media server: ' + e.message, 'error');
      endCall();
    }
  }
  ...
  async function endCall() {
    ...
    document.getElementById('active-call-overlay').classList.remove('fullscreen');
    const remoteVideo = document.getElementById('remote-video');
    const localVideo = document.getElementById('local-video');
    remoteVideo.srcObject = null;
    localVideo.srcObject = null;
    remoteVideo.classList.remove('pip-swapped');
    localVideo.classList.remove('pip-swapped');
    ...
    document.getElementById('share-screen').style.background = 'rgba(255,255,255,0.1)';
  ...
  document.getElementById('toggle-mic').onclick = async () => {
    if (lkRoom) {
      const isEnabled = lkRoom.localParticipant.isMicrophoneEnabled;
      await lkRoom.localParticipant.setMicrophoneEnabled(!isEnabled);
      document.getElementById('toggle-mic').style.background = !isEnabled ? 'rgba(255,255,255,0.1)' : 'var(--danger)';
    }
  };

  document.getElementById('toggle-video').onclick = async () => {
    if (lkRoom) {
      const isEnabled = lkRoom.localParticipant.isCameraEnabled;
      await lkRoom.localParticipant.setCameraEnabled(!isEnabled);
  ...
  async function shareScreen() {
    ...
    try {
      ...
      shareBtn.style.background = !isSharing ? 'var(--accent)' : 'rgba(255,255,255,0.1)';
      ...
      if (!isSharing) {
        ...
        // Stopped sharing
        const indicator = document.getElementById('local-screen-share-indicator');
        if (indicator) indicator.remove();
        showNotification('Screen Sharing', 'Screen sharing stopped', 'info');
      }
    } catch (e) {
      console.error('Screen share error:', e);
      showNotification('Error', 'Could not share screen: ' + e.message, 'error');
    }
  }
  ...
  Object.assign(window.app, {
    openChannel, openDm, openFriendsView,
  ...
  function startApp() {
    ...
    // Mobile Menu Toggle
    document.getElementById('chat-banner')?.addEventListener('click', (e) => {
      if (window.innerWidth <= 800) {
        const rect = e.currentTarget.getBoundingClientRect();
        if (e.clientX - rect.left < 60) {
          document.body.classList.toggle('sidebars-open');
        }
      }
    });

    // Close sidebar when clicking main content
    document.getElementById('chat-view')?.addEventListener('click', () => {
      if (window.innerWidth <= 800 && document.body.classList.contains('sidebars-open')) {
    ...
    // PIP Swap Logic
    const localVideoPIP = document.getElementById('local-video');
    if (localVideoPIP) {
      localVideoPIP.onclick = (e) => {
        e.stopPropagation();
        const localVideo = document.getElementById('local-video');
        const remoteVideo = document.getElementById('remote-video');

        localVideo.classList.toggle('pip-swapped');
        remoteVideo.classList.toggle('pip-swapped');
      };
    }

    // Share Screen
    const shareScreenBtn = document.getElementById('share-screen');
    if (shareScreenBtn) shareScreenBtn.onclick = shareScreen;
    ...
    window.app.showJoinByCodeModal = () => {
      document.getElementById('modal-container').style.display = 'flex';
      document.getElementById('modal-title').textContent = 'Join Server';
      document.getElementById('modal-body').innerHTML = `
          <div class="form-group">
            <label>Invite Code</label>
            <input id="m-join-code" placeholder="ENTER-CODE" style="width:100%;padding:10px;text-transform:uppercase;">
            <p style="font-size:0.75rem;color:var(--text-muted);margin-top:8px;">Enter a 6-character short code or a server ID to join.</p>
          </div>
        `;
      document.getElementById('modal-confirm').style.display = 'block';
      document.getElementById('modal-confirm').textContent = 'Join Server';
      document.getElementById('modal-confirm').onclick = async () => {
        ...
        if (code) {
          try {
            const res = await api('/api/servers/join', { method: 'POST', body: { code } });
            document.getElementById('modal-container').style.display = 'none';
            showNotification('Success', 'You joined the server!', 'success');
            window.app.renderSidebarNav();
            window.app.openServer(res.serverId);
          } catch (e) {
            alert(e.message);
          }
        }
    ...
    const resumeAudio = () => {
      console.log("[Audio] Interaction detected - checking media states");
      document.querySelectorAll('video, audio').forEach(el => {
        if (el.paused && el.srcObject) {
          el.play().catch(() => { });
        }
      });
      // Also broadcast a custom event that components can listen to
      window.dispatchEvent(new CustomEvent('user-interacted'));
