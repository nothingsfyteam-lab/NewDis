...
router.post('/', auth, (req, res) => {
    ...
    try {
        const txn = db.transaction(() => {
            db.prepare('INSERT INTO servers (id, name, owner_id, icon, invite_code) VALUES (?, ?, ?, ?, ?)').run(serverId, name, req.session.userId, icon || '', inviteCode);
...
router.post('/join', auth, (req, res) => {
    ...
    try {
        // Support both full server ID and short invite code
        const server = db.prepare('SELECT id FROM servers WHERE id = ? OR invite_code = ?').get(code, code.toUpperCase());
